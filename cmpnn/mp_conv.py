...
class etype_net(torch.nn.Module):
    def __init__(self, nedge_type, nmid_filter=64, nfeature_size=3):
        super(etype_net, self).__init__()
        self.main = torch.nn.Sequential(
            torch.nn.Conv2d(nfeature_size, nmid_filter, 1, 1),
            torch.nn.BatchNorm2d(nmid_filter), torch.nn.ReLU(inplace=True),
...
class gconv_residual(torch.nn.Module):
    ...
    def forward(self, node_feature, etype, nn_idx):
